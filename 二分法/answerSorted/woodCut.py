...
def woodCut(array,k):
    ...
    while start+1<end:
        mid = (start+end)//2
        if getPieces(array,mid)>k:
            start =mid
        elif getPieces(array,mid)<k:
            end = mid
        else:
            start =mid
    if getPieces(array, start) >= k:
        return start
    if getPieces(array,end)>=k:
        return end



def getPieces(array,length):
    piece = 0
    for wood in array:
        piece += wood//length
    return piece
print(woodCut([32,12,45],7))
